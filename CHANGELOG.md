...
- Added `examples/basic-synth`, with a framework-independent core plus vanilla
  TypeScript and Svelte 5/Vite UI adapters for the same monophonic C4–C5
  instrument. It provides volume and low-pass cutoff controls, release tails, a
  single Power on / Power off control, asset-error recovery, active/held-note
  feedback, bounded keyboard navigation, touch scrolling, and desktop keycaps.
  Both adapters import only the public browser package and use the same shared
  audio owner, graph, keyboard reducer, styles, package lock, and browser
  lifetime scenarios. The Svelte adapter replaces selector configuration and
  manual DOM projection with declarative state, events, and keyed piano markup.
- Added named graph parameters to the browser facade: descriptions can declare
  ...
  adapters use named `volume` and `cutoff` updates while retaining raw atomic note
...
- Added `npm run pack:browser` to build a local `@moondsp/browser` tarball
  ...
  Consumers can build and deploy either adapter without a MoonBit toolchain.
...
- Consolidated snapshot playback into `PatternScheduler::render_block` for dry
  ...
  rendering aliases and migrated callers. Separate send buffer arguments avoid
  per-call tuple construction; callers no longer pre-clear route outputs.
...
- Both synth adapters use a discriminated lifecycle state and one resource owner
...
- Both synth adapters have one Power on / Power off button, with no separate Start,
