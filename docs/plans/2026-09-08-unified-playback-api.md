...
`browser/internal/playback_host/live_update.mbt` に、明示的な入力から準備結果を作る処理と、純粋な受理・遷移判断を置く。準備slot、保留中操作、transport、voice lifecycleはaudio-owner shellが所有する。通常再生も再スタートも `render_block` を使う。
