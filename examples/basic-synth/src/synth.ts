...
export const SYNTH_GRAPH = {
  nodes: [
    ...
    { type: "biquad", input: OSCILLATOR_NODE, mode: "lowpass", cutoff: DEFAULT_SETTINGS.cutoff, q: 0.7 },
    { type: "adsr", attackMs: 10, decayMs: 80, sustain: 0.7, releaseMs: 180 },
    { type: "mul", input0: FILTER_NODE, input1: ADSR_NODE },
    { type: "gain", input: 3, gain: DEFAULT_SETTINGS.volume },
    { type: "output", input: GAIN_NODE },
  ],
} as const satisfies GraphDescription;

export function volumeControl(value: number): GraphControl {
  return { type: "setParam", node: GAIN_NODE, slot: "value0", value };
}

export function cutoffControl(value: number): GraphControl {
  return { type: "setParam", node: FILTER_NODE, slot: "value0", value };
}
